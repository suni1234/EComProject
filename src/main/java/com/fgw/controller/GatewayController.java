...
public class GatewayController {
    ...
    public ResponseEntity<Map<String, Object>> profile(Authentication auth) {
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "email",    tokenValidator.getEmail(claims),
                "sub",      tokenValidator.getSub(claims),
     ...
     *
     * curl -H "Authorization: Bearer <idToken>" http://localhost:8080/api/token-info
     */
    ...
    public ResponseEntity<Map<String, Object>> tokenInfo(Authentication auth) {
        ...
        return ResponseEntity.ok(Map.of(
                ...
                "token_use",  claims.getOrDefault("token_use", "N/A"),
                "groups",     tokenValidator.getGroups(claims),
                "email",      String.valueOf(claims.getOrDefault("email", "N/A"))
        ));
    }
     ...
     *
     * curl -H "Authorization: Bearer <idToken>" http://localhost:8080/api/credentials
     */
     ...
     *
     * curl -H "Authorization: Bearer <adminIdToken>" http://localhost:8080/api/admin/dashboard
     */
